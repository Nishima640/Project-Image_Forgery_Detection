...
def buildTable(descriptor,kp):
        ...
        TABLE=np.zeros((image_width,image_height),np.int64)
        TABLE1=np.zeros((image_width,image_height),np.int64)
        TABLE2=np.zeros((image_width,image_height),np.int64)
        TABLE=TABLE.tolist()
...
def calculateAccuracy(f1, f2, ground_truth):
    true_positive = 0
    false_positive = 0
    false_negative = 0
    true_negative = 0

    # Check if the ground truth image is not None
    if ground_truth is None:
        print("Error: ground truth image is None.")
        return None
    # Check if the keypoints are not None
    ...
    for kp1, kp2 in zip(f1, f2):
        x1, y1 = kp1.pt
        x2, y2 = kp2.pt
        if ground_truth[int(y1), int(x1)] == ground_truth[int(y2), int(x2)]:
            if ground_truth[int(y1), int(x1)] == 255:
                true_positive += 1    
            else:
                true_negative += 1
        else:
            if ground_truth[int(y2), int(x2)] == 255:   
                false_positive += 1
            else:
                false_negative += 1 
...
def main(file_name):
    ...
    ground_truth = cv2.imread("im1_t_mask.png", cv2.IMREAD_GRAYSCALE)
    try:
        accuracy, precision, recall= calculateAccuracy(f1, f2, ground_truth)
    except TypeError:
